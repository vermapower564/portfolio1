            ...
            {navItems.map((item) => {
              ...
              return (
                <a
                  key={item.href}
                  href={item.href}
                  className={`px-4 py-1.5 rounded-full text-xs font-mono font-medium transition-all duration-200 focus:outline-none focus:ring-2 focus:ring-sky-500 ${
                    isActive
        ...
        <div className="md:hidden fixed inset-0 top-[65px] bg-slate-950/95 backdrop-blur-xl z-40 border-b border-slate-800 animate-in fade-in slide-in-from-top-4 duration-200 flex flex-col justify-between p-6">
          ...
          <div className="pt-6 border-t border-slate-800 space-y-3">
            ...
            <div className="text-center font-mono text-xs text-slate-500">
              Roushan Kumar Verma — Full-Stack Developer
            </div>
          </div>
        </div>
      )}
